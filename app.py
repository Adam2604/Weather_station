...
def index():
    ...
    <body>
        ...
        <div class="card">
            <a href="/wilgotnosc">Wilgotność: <span class="value">{{ wilgotnosc }}</span> %</a>
        </div>
    </body>
    </html>
    """
...
def temperatura_page():
    ...
    labels = [row[1].replace(" ", "T") for row in rows]
...
def wilgotnosc_page():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT wilgotnosc, timestamp FROM pomiary WHERE date(timestamp)=? ORDER BY timestamp",
        (today,)
    )
    rows = cursor.fetchall()
    conn.close()
    ...
    <body style="display:flex; flex-direction:column; align-items:center; justify-content:center;">
        <h2 style="margin-bottom: 10px;">Wykres wilgotności z dzisiejszego dnia</h2>
